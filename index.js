...
async function updateProject(req, res) {
    ...
    try {
        let currentProject = await pool.query("SELECT * FROM projects WHERE id = $1", [id]);
        ...
        await pool.query(
            ...
            WHERE id = $10`,
            [name, start_date, end_date, description, nodejs, nextjs, reactjs, typescript, upload_image, id]
        );
        res.redirect("/");
    } catch (err) {
        console.error(err);
